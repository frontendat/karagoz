...
1. `suppressAddressBar`: when true, the address bar above the preview frame is not rendered.
2. `suppressConsole`: when true, the console toggle button next to the address bar's reload icon is not rendered.
   The console panel, which shows `console.log`/`info`/`warn`/`error`/`debug` output and uncaught errors from the
   preview, can still be controlled programmatically regardless of this option, through `sandbox.preview.showConsole()`
   and `sandbox.preview.hideConsole()`. Its current state can be read from `sandbox.preview.consoleShown`.
